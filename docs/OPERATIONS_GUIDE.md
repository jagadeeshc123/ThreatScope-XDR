...
SOAR delays and retry waits are persistent and non-blocking. Call `POST /api/soar/executions/process-due` with a batch size from 1 to 100 using an authenticated CSRF-protected `soar:execute` session. Repeated calls are safe and oldest-first; no external scheduler is installed. Diagnostics report catalog/policy/template integrity, unknown keys, invalid active playbooks, pending approvals, stuck/expired executions, overdue delays, and rollback failures. Backups and restore validation require Phase 16 sentinel tables and preserve active executions, approvals, inputs, evidence, versions, and rollback records. Retention is preview-first: terminal dry runs 90 days, simulations 180 days, live-local executions 365 days, and reports 365 days; pending and case-triggered executions and all immutable playbook versions are preserved. Demo reset removes only demo-owned SOAR clones, executions and dependent workflow rows, and demo reports; protected templates and production policies remain.
