...
SOAR-Lite dispatch is a fixed backend allowlist with five exact classifications. Declarative playbooks cannot name a callable, module, command, SQL, URL, webhook, filesystem path, or arbitrary variable root. Sensitive local user/session actions always require Administrator approval, explicit reason, dispatch-time permission, current-state reload, last-active-Administrator protection, and separation where another eligible Administrator exists. Simulation-only actions never permit live-local mode or contact external infrastructure. Execution, approval, analyst-input, evidence, compensation, activity, notifications, and hash-chained audit metadata are bounded and redacted.

# Integration Hub safety boundary

Phase 17 connector definitions are immutable backend catalog entries. Credentials are encrypted and write-only; configuration, responses, audit records, reports, and frontend state never return secret material. Outbound traffic requires an active connector, a validated exact host/port policy, verified TLS, bounded payloads and responses, and DNS-answer validation immediately before a pinned connection. Redirects and private, loopback, link-local, multicast, reserved, and metadata destinations are blocked unless an Administrator explicitly approves an exact private destination. Inbound events require bounded HMAC timestamp/nonce verification and enter quarantine before explicit promotion. Declarative mappings cannot execute code, SQL, templates, shell commands, or unbounded expressions.

# Production security boundary

Production fails closed on unsafe configuration and accepts required secrets only through bounded regular-file mounts. Exact hosts/origins, trusted-proxy CIDRs, secure HttpOnly SameSite cookies, CSRF, minimal public health, protected diagnostics, request IDs, redacted logs, request bounds, and TLS edge controls provide defense in depth. Containers run rootless, read-only, unprivileged, capability-free, resource-limited, segmented, and without the Docker socket. Startup/preflight performs no external request. Phase 16 response actions remain simulations unless already defined as safe local state operations, and Phase 18 anomalies remain analyst-review signals rather than confirmed incidents or containment triggers.
