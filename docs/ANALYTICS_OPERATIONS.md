...
Database schema sentinel 18 includes detectors, versions, baselines, jobs, backtests, evaluations, anomalies, contributions, feedback, suppressions, drift, and reports. Startup creates missing tables through the existing metadata flow. Backup inventory, staged restore validation, retention preview/apply, diagnostics, activity, notifications, audit, demo reset, dashboard, and search include analytics records.
