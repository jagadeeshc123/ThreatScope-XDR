...
export function Dashboard() {
  ...
  const limitedAccount = !!user?.roles.includes('registered_user') && !user.permissions.some(permission => ['web:read','api:read','soc:read','document:read','phishing:read','correlation:read','cases:read','governance:read','operations:view','integrations:view'].includes(permission));
  ...
  return (
    <PageShell>
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><StatCard label="Active IOCs" value={summary.threat_intel_active_indicators} detail="Offline threat-intelligence inventory" icon={<Radar className="h-5 w-5"/>} tone="info"/><StatCard label="High-Risk IOC Matches" value={summary.threat_intel_high_risk_matches} detail="Stored-data correlation" icon={<ShieldAlert className="h-5 w-5"/>} tone="danger"/><StatCard label="Recent IOC Imports" value={summary.threat_intel_recent_imports} detail="Last seven days" icon={<FileText className="h-5 w-5"/>} tone="default"/><StatCard label="Recent IOC Escalations" value={summary.threat_intel_recent_escalations} detail="Explicit case links" icon={<AlertTriangle className="h-5 w-5"/>} tone="warn"/></div>
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5"><StatCard label="Active Detection Rules" value={summary.detection_active_rules} detail="Validated stored-data rules" icon={<Radar className="h-5 w-5"/>} tone="good"/><StatCard label="High-Risk Detections" value={summary.detection_high_risk_matches} detail="Unsuppressed matches" icon={<ShieldAlert className="h-5 w-5"/>} tone="danger"/><StatCard label="Failed Validations" value={summary.detection_failed_validations} detail="Rules requiring attention" icon={<AlertTriangle className="h-5 w-5"/>} tone="warn"/><StatCard label="ATT&CK Subset Coverage" value={`${summary.detection_attack_coverage}%`} detail="Bounded local educational subset" icon={<Shield className="h-5 w-5"/>} tone="info"/><StatCard label="Detection Escalations" value={summary.detection_recent_escalations} detail="Explicit case promotion" icon={<FileText className="h-5 w-5"/>} tone="default"/></div>
      {summary.vulnerability_management&&<div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5"><StatCard label="Active Vulnerabilities" value={summary.vulnerability_management.active_vulnerabilities} detail="Local stored-data queue" icon={<ShieldAlert className="h-5 w-5"/>} tone="warn"/><StatCard label="Critical / High VM" value={summary.vulnerability_management.critical_high_vulnerabilities} detail="Technical severity" icon={<AlertTriangle className="h-5 w-5"/>} tone="danger"/><StatCard label="Overdue Vulnerabilities" value={summary.vulnerability_management.overdue_vulnerabilities} detail="Deterministic SLA" icon={<Activity className="h-5 w-5"/>} tone="danger"/><StatCard label="Due Soon" value={summary.vulnerability_management.due_within_seven_days} detail="Next seven days" icon={<FileText className="h-5 w-5"/>} tone="warn"/><StatCard label="Unassigned Vulnerabilities" value={summary.vulnerability_management.unassigned_vulnerabilities} detail="Needs owner" icon={<Shield className="h-5 w-5"/>} tone="info"/></div>}
      {summary.soar&&<div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><StatCard label="Pending SOAR Approvals" value={summary.soar.pending_approvals} detail="Explicit governance decisions" icon={<ShieldAlert className="h-5 w-5"/>} tone="warn"/><StatCard label="Failed SOAR Executions" value={summary.soar.failed_executions} detail="Preserved failure history" icon={<AlertTriangle className="h-5 w-5"/>} tone="danger"/><StatCard label="SOAR Analyst Inputs" value={summary.soar.waiting_analyst_inputs} detail="Waiting human review" icon={<Activity className="h-5 w-5"/>} tone="info"/><StatCard label="SOAR Simulations" value={summary.soar.simulation_count} detail="No external action performed" icon={<Shield className="h-5 w-5"/>} tone="info"/></div>}
      {summary.integrations&&<div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><StatCard label="Unhealthy Connectors" value={summary.integrations.unhealthy_connectors} detail={`${summary.integrations.open_circuits} open circuits`} icon={<AlertTriangle className="h-5 w-5"/>} tone="danger"/><StatCard label="Failed Deliveries" value={summary.integrations.failed_deliveries} detail={`${summary.integrations.due_retries} due retries`} icon={<Activity className="h-5 w-5"/>} tone="warn"/><StatCard label="Dead Letters" value={summary.integrations.dead_letters} detail="Manual replay only" icon={<FileText className="h-5 w-5"/>} tone="danger"/><StatCard label="Inbound Quarantine" value={summary.integrations.quarantined_events} detail={`${summary.integrations.rejected_signatures} rejected signatures`} icon={<ShieldAlert className="h-5 w-5"/>} tone="warn"/></div>}
