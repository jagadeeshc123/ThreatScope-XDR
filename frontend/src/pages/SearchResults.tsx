...
export function SearchResultsPage() {
  ...
  const totalResults = results ? Object.values(results).reduce((total,value)=>total+(Array.isArray(value)?value.length:0),0) : 0;
    ...
    <PageShell className="max-w-5xl">
      ...
      {!loading && !error && totalResults > 0 && results && (
        <div className="space-y-8">
          ...
          <ResultGroup title="Detection Rules" icon={<Radar className="h-5 w-5 text-cyan-300"/>} count={results.detection_rules.length}>{results.detection_rules.map(x=><ResultLink key={x.id} to={`/detections/rules/${x.id}`} title={x.title} detail={`${x.severity} · ${x.lifecycle_status} · quality ${x.quality_score}`}/>)}</ResultGroup>
          <ResultGroup title="Detection Packs" icon={<Shield className="h-5 w-5 text-blue-300"/>} count={results.detection_packs.length}>{results.detection_packs.map(x=><ResultLink key={x.id} to={`/detections/packs/${x.id}`} title={x.name} detail={`${x.version} · ${x.enabled?'enabled':'disabled'}`}/>)}</ResultGroup>
          <ResultGroup title="ATT&CK Techniques" icon={<Network className="h-5 w-5 text-violet-300"/>} count={results.attack_techniques.length}>{results.attack_techniques.map(x=><ResultLink key={x.id} to={`/detections/coverage?technique=${x.external_id}`} title={`${x.external_id} · ${x.name}`} detail={`${x.tactic} · bounded local subset`}/>)}</ResultGroup>
          <ResultGroup title="Detection Matches" icon={<ShieldAlert className="h-5 w-5 text-red-300"/>} count={results.detection_matches.length}>{results.detection_matches.map(x=><ResultLink key={x.id} to={`/detections/matches/${x.id}`} title={`Match ${x.id} · risk ${x.risk_score}`} detail={`${x.status} · ${x.snippet}`}/>)}</ResultGroup>
          <ResultGroup title="Detection Executions" icon={<Activity className="h-5 w-5 text-emerald-300"/>} count={results.detection_executions.length}>{results.detection_executions.map(x=><ResultLink key={x.id} to={`/detections/executions/${x.id}`} title={`Execution ${x.id} · ${x.status}`} detail={`${x.records_scanned} scanned · ${x.matches_found} matches`}/>)}</ResultGroup>
          <ResultGroup title="Detection Suppressions" icon={<Shield className="h-5 w-5 text-amber-300"/>} count={results.detection_suppressions.length}>{results.detection_suppressions.map(x=><ResultLink key={x.id} to="/detections/suppressions" title={x.name} detail={`${x.enabled?'enabled':'disabled'} · ${x.description}`}/>)}</ResultGroup>
          <ResultGroup title="Detection Reports" icon={<FileText className="h-5 w-5 text-fuchsia-300"/>} count={results.detection_reports.length}>{results.detection_reports.map(x=><ResultLink key={x.id} to={`/detections/reports/${x.id}`} title={x.title} detail={x.report_type}/>)}</ResultGroup>
          <ResultGroup title="Assets" icon={<Target className="h-5 w-5 text-cyan-300"/>} count={results.vm_assets.length}>{results.vm_assets.map(x=><ResultLink key={x.id} to={x.internal_path} title={x.name} detail={`${x.asset_type} · ${x.criticality} · ${x.environment}`}/>)}</ResultGroup>
          <ResultGroup title="Vulnerabilities" icon={<ShieldAlert className="h-5 w-5 text-red-300"/>} count={results.vm_vulnerabilities.length}>{results.vm_vulnerabilities.map(x=><ResultLink key={x.id} to={x.internal_path} title={x.title} detail={`${x.severity} · priority ${x.priority_score} · ${x.status}`}/>)}</ResultGroup>
          <ResultGroup title="Remediation Plans" icon={<Shield className="h-5 w-5 text-emerald-300"/>} count={results.vm_remediation_plans.length}>{results.vm_remediation_plans.map(x=><ResultLink key={x.id} to={x.internal_path} title={x.title} detail={`${x.priority} · ${x.status}`}/>)}</ResultGroup>
          <ResultGroup title="Remediation Tasks" icon={<Activity className="h-5 w-5 text-blue-300"/>} count={results.vm_remediation_tasks.length}>{results.vm_remediation_tasks.map(x=><ResultLink key={x.id} to={x.internal_path} title={x.title} detail={x.status}/>)}</ResultGroup>
          <ResultGroup title="SLA Policies" icon={<Activity className="h-5 w-5 text-amber-300"/>} count={results.vm_sla_policies.length}>{results.vm_sla_policies.map(x=><ResultLink key={x.id} to={x.internal_path} title={x.name} detail={`${x.target_days} days · ${x.enabled?'enabled':'disabled'}`}/>)}</ResultGroup>
          <ResultGroup title="Risk Acceptances" icon={<AlertTriangle className="h-5 w-5 text-orange-300"/>} count={results.vm_risk_acceptances.length}>{results.vm_risk_acceptances.map(x=><ResultLink key={x.id} to={x.internal_path} title={`Acceptance #${x.id}`} detail={`${x.residual_risk} residual risk · ${x.status}`}/>)}</ResultGroup>
          <ResultGroup title="Verification Requests" icon={<Shield className="h-5 w-5 text-cyan-300"/>} count={results.vm_verifications.length}>{results.vm_verifications.map(x=><ResultLink key={x.id} to={x.internal_path} title={`Verification #${x.id}`} detail={`${x.verification_type} · ${x.status}`}/>)}</ResultGroup>
          <ResultGroup title="Remediation Templates" icon={<FileText className="h-5 w-5 text-violet-300"/>} count={results.vm_remediation_templates.length}>{results.vm_remediation_templates.map(x=><ResultLink key={x.id} to={x.internal_path} title={x.title} detail={`${x.category} · ${x.system_owned?'system':'custom'}`}/>)}</ResultGroup>
          <ResultGroup title="Vulnerability Reports" icon={<FileText className="h-5 w-5 text-fuchsia-300"/>} count={results.vm_reports.length}>{results.vm_reports.map(x=><ResultLink key={x.id} to={x.internal_path} title={x.title} detail={x.report_type}/>)}</ResultGroup>
          <ResultGroup title="SOAR Playbooks and Templates" icon={<Shield className="h-5 w-5 text-cyan-300"/>} count={results.soar_playbooks.length}>{results.soar_playbooks.map(x=><ResultLink key={x.id} to={x.internal_path} title={x.title} detail={`${x.kind} · ${x.status}`}/>)}</ResultGroup>
          <ResultGroup title="SOAR Trigger Rules" icon={<Radar className="h-5 w-5 text-blue-300"/>} count={results.soar_triggers.length}>{results.soar_triggers.map(x=><ResultLink key={x.id} to={x.internal_path} title={x.title} detail={x.status}/>)}</ResultGroup>
          <ResultGroup title="SOAR Executions" icon={<Activity className="h-5 w-5 text-emerald-300"/>} count={results.soar_executions.length}>{results.soar_executions.map(x=><ResultLink key={x.id} to={x.internal_path} title={x.title} detail={`${x.mode} · ${x.status}`}/>)}</ResultGroup>
          <ResultGroup title="SOAR Approvals" icon={<ShieldAlert className="h-5 w-5 text-amber-300"/>} count={results.soar_approvals.length}>{results.soar_approvals.map(x=><ResultLink key={x.id} to={x.internal_path} title={x.title} detail={x.status}/>)}</ResultGroup>
          <ResultGroup title="SOAR Analyst Inputs" icon={<FileText className="h-5 w-5 text-violet-300"/>} count={results.soar_analyst_inputs.length}>{results.soar_analyst_inputs.map(x=><ResultLink key={x.id} to={x.internal_path} title={x.title} detail={x.status}/>)}</ResultGroup>
