...
from app.modules.platform_operations.backup_service import _safe_counts, materialize_plain
...
def main(argv=None):
    args=parser().parse_args(argv);source=database_path()
    ...
    with SessionLocal() as db:
        ...
        # The candidate must share the database filesystem so the final rename is atomic.
        # Keeping it in runtime_dir breaks when production uses separate named volumes.
        temp=source.with_suffix(source.suffix+".restore-candidate")
        ...
        backup_identifier=backup.id;backup_key=backup.backup_key;safety_identifier=safety.id
    engine.dispose()
    ...
    with SessionLocal() as db:
        ...
        restore=db.query(RestoreRecord).filter_by(backup_id=backup_identifier).order_by(RestoreRecord.created_at.desc()).first()
        if restore: restore.status="succeeded";restore.pre_restore_backup_id=safety_identifier;restore.completed_at=utcnow();db.commit()
        append_event(db,event_type="database_restored",action="offline_restore",request_id="local-restore-script",outcome="success",resource_type="backup",resource_id=backup_key,status_code=200,metadata={"sessions_revoked":sessions,"pre_restore_backup_id":safety_identifier})
