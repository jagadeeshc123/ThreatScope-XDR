...
def delete_scan(scan_id: int, db: Session = Depends(get_db)):
    ...
    if scan.status in {"queued", "running"}:
        raise HTTPException(status_code=409, detail="Active scans cannot be deleted")

    report_ids = [row.id for row in db.query(models.Report.id).filter(models.Report.scan_id == scan_id).all()]
    db.query(models.Notification).filter(or_(
        and_(models.Notification.entity_type == "scan", models.Notification.entity_id == scan_id),
        and_(models.Notification.entity_type == "report", models.Notification.entity_id.in_(report_ids)),
    )).delete(synchronize_session=False)
    db.query(models.PostureDiff).filter(or_(
        models.PostureDiff.current_scan_id == scan_id,
        models.PostureDiff.previous_scan_id == scan_id,
    )).delete(synchronize_session=False)
    db.query(models.EvidenceArtifact).filter(models.EvidenceArtifact.scan_id == scan_id).delete(synchronize_session=False)
    db.query(models.CrawlNode).filter(models.CrawlNode.scan_id == scan_id).delete(synchronize_session=False)
    db.query(models.Finding).filter(models.Finding.scan_id == scan_id).delete(synchronize_session=False)
    db.query(models.Report).filter(models.Report.scan_id == scan_id).delete(synchronize_session=False)
    db.delete(scan)
    db.commit()
    return {"ok": True}
