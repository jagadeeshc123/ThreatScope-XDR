...
def reset(db: Session) -> dict:
    ...
    return {**status(db), "deleted_demo_records": count + len(demo_vm_assets) + len(demo_indicators) + len(demo_sources) + len(demo_rules) + len(demo_detection_executions) + len(demo_executions) + len(demo_playbooks) + demo_soar_reports + len(demo_connectors) + demo_integration_reports, "deleted_demo_vulnerability_management_records": len(demo_vm_assets), "deleted_demo_threat_intel_records": len(demo_indicators) + len(demo_sources), "deleted_demo_detection_records": len(demo_rules) + len(demo_detection_executions), "deleted_demo_soar_records": len(demo_executions) + len(demo_playbooks) + demo_soar_reports, "deleted_demo_integration_records":len(demo_connectors)+len(demo_deliveries)+len(demo_endpoints)+demo_integration_reports, "non_demo_records_preserved": True, "protected_soar_templates_preserved": True, "real_connector_credentials_preserved":True}
