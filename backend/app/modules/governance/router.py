...
def sync_risks(source_module:str|None=None,minimum_source_severity:str="info",since:datetime|None=None,maximum_records_per_module:int=Query(500,ge=1,le=1000),include_existing_closed_source_records:bool=False,db:Session=Depends(get_db)):return risk_service.synchronize(db,source_module,minimum_source_severity,since,maximum_records_per_module,include_existing_closed_source_records)
@router.get("/risks")
...
def generate_mappings(payload:dict|None=None,db:Session=Depends(get_db)):
    payload=payload or {};return mapping_engine.generate(db,payload.get("risk_ids"),payload.get("source_modules"),payload.get("framework_keys"),payload.get("minimum_confidence","low"))
@router.get("/mappings")
...
def create_treatment(risk_id:int,payload:dict,db:Session=Depends(get_db)):
    ...
    if payload.get("strategy") not in {"mitigate","avoid","transfer","monitor"}:raise HTTPException(422,"Unsupported treatment strategy")
    for field in ("expected_residual_likelihood","expected_residual_impact"):
        if payload.get(field) is not None and int(payload[field]) not in range(1,6):raise HTTPException(422,f"{field} must be between 1 and 5")
...
def request_exception(risk_id:int,payload:dict,db:Session=Depends(get_db)):
    ...
    if not justification:raise HTTPException(422,"Exception justification is required")
...
def create_package(payload:dict,db:Session=Depends(get_db)):
    ...
    if not title:raise HTTPException(422,"Evidence package title is required")
...
def create_review(payload:dict,db:Session=Depends(get_db)):
    ...
    if not title:raise HTTPException(422,"Review title is required")
